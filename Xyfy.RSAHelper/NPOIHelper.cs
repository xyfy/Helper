    ...
    public class NPOIHelper<T> : IDisposable where T : class, new()
    {
        ...
        public void ExportToWeb(System.Web.HttpResponse response, List<T> data, int beginRow, string fileName)
        {
            byte[] msByte = null;
            if (WriteData(data, beginRow))
            {
                msByte = WriteToMemory();
            }
            // 设置编码和附件格式
...
            //#endregion
            foreach (var item in data)
            {
                ...
                for (int j = 0; j < propertyNames.Length; j++)
                {
                    ...
                    var icell = this.GetCell(currentRowIndex, cellIndex);
